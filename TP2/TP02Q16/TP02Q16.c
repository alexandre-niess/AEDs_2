...
int main() {
    ...
    // Ordena em ordem crescente de ano de nascimento
    for (int i = 0; i < len - 1; i++) {
        for (int j = 0; j < len - i - 1; j++) {
            if (strcmp(array[j].birth_year, array[j+1].birth_year) > 0) {
                SWCharacter temp = array[j];
                array[j] = array[j+1];
                array[j+1] = temp;
            }
        }
    ...
    // Imprime os personagens na ordem ordenada
    for(size_t i = 0; i < len; i++) {
        swc_print(&array[i]);
    }
