...
public class TP02Q07 {
	...
	public static void main (String args[]) throws Exception{
		...
		for(int i = 0; i < numEntrada; i++){
			nomes[i] = personagens[i].getNome();
		} // adiciona os nomes dos personagens em um array para realizar a busca sequencial
		...
		for(int i=0;i<numEntrada2;i++){
			String chave=entrada2[i];
			if (buscaSequencial(nomes, chave, numEntrada) == 1) {
	...
	public static int buscaSequencial(String[] nomes, String chave, int tam) {
		for (int i = 0; i < tam; i++) {
			if (nomes[i].equals(chave)) {
				return 1; // retorna o índice onde a chave foi encontrada
			}
		}
		return 0; // retorna -1 caso a chave não tenha sido encontrada
	}
}
