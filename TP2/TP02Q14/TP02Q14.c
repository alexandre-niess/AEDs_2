...
int compare_hair_color(const void* a, const void* b) {
    const SWCharacter* swc1 = (const SWCharacter*)a;
    const SWCharacter* swc2 = (const SWCharacter*)b;
    return strcmp(swc1->hair_color, swc2->hair_color);
}
...
int main() {
    ...
    qsort(array, len, sizeof(SWCharacter), compare_hair_color);
    for(size_t i = 0; i < len; i++) {
        swc_print(&array[i]);
    }
