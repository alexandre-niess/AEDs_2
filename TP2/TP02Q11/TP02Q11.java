...
public class TP02Q11 {
    ...
    public static void insertionSort(Personagem[] lista) throws Exception {
        int n = lista.length;
        for (int i = 1; i < n; i++) {
            ...
            while (j >= 0 && lista[j].getAnoNascimento().compareTo(tmp.getAnoNascimento()) > 0) {
                ...
                lista[j + 1] = lista[j];
                //lista.inserir(lista.get(j), j + 1); // Deslocamento
                j--;
            }
            lista[j + 1] = tmp;
            //lista.inserir(tmp, j + 1);
        }
    }
}
